...
interface Props {
  ...
  isRollMode?: boolean
  student: Person,
  onAttendance?: (id: any ,newState: RolllStateType) => void,
}
export const StudentListTile: React.FC<Props> = ({ isRollMode, student,  onAttendance}) => {

  const onStage = (id: number, value: any) => {

    if(onAttendance) {
      onAttendance(id, value);
    }
  }
  return (
    <S.Container>
      ...
      {isRollMode && (
        <S.Roll>
          <RollStateSwitcher onStateChange={onStage} student={student}/>
        </S.Roll>
      )}
    </S.Container>
  )
